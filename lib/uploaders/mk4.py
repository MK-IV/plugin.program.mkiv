...
Local      =  MK4Build   =  xbmc.translatePath(os.path.join('special://home/addons/',addon_id))
Master      =  MK4Build   =  xbmc.translatePath(os.path.join('special://home/addons/',addon_id+'-master'))
...
RequestsModule = xbmc.translatePath(os.path.join('special://home/addons/'+addon_id+'/resources/','script.module.requests.zip'))
    ...
    for root, dirs, files in os.walk(url):  #Search all xml and script.skinshortcuts files and replace physical with special
        for file in files:
            if file.endswith(".xml"):
                 ...
                 b=a.replace(USERDATA, 'special://home/userdata/').replace(ADDONS,'special://home/addons/').replace(MEDIA,'special://home/media/')
...
def Restore():
        ...
        try: 
            ...
            xbmc.log(''+e+'')
        dp.update(0,"Extracting and Writing Files... [COLOR lime]DONE[/COLOR]", "Checking paths and cleaning up...")
        time.sleep(.5)
        FIX_SPECIAL(HOME)
        time.sleep(.5)
        #xbmc.executebuiltin('UpdateLocalAddons')
...
def MakeRssFile():
    ...
    if ( not vqname ): return False, 0
    name = urllib.unquote_plus(vqname)
    xbmcgui.Dialog().ok(Title,'You will be prompted to add ten lines to your RSS feed.','If want less than 10 just hit "Done" on the keyboard','to skip through the rest.'  )  
    vqline1 = _get_keyboard(heading="Enter line 1 of your feed" )
    if ( not vqline1 ): return False, 0
    line1 = urllib.unquote_plus(vqline1)
    vqline2 = _get_keyboard(heading="Enter line 2 of your feed" )
    if ( not vqline2 ): vqline2 = ""
    line2 = urllib.unquote_plus(vqline2)
    vqline3 = _get_keyboard(heading="Enter line 3 of your feed" )
    if ( not vqline3 ): vqline3 = ""
    line3 = urllib.unquote_plus(vqline3)
    vqline4 = _get_keyboard(heading="Enter line 4 of your feed" )
    if ( not vqline4 ): vqline4 = ""
    line4 = urllib.unquote_plus(vqline4)
    vqline5 = _get_keyboard(heading="Enter line 5 of your feed" )
    if ( not vqline5 ): vqline5 = ""
    line5 = urllib.unquote_plus(vqline5)
    vqline6 = _get_keyboard(heading="Enter line 6 of your feed" )
    if ( not vqline6 ): vqline6 = ""
    line6 = urllib.unquote_plus(vqline6)
    vqline7 = _get_keyboard(heading="Enter line 7 of your feed" )
    if ( not vqline7 ): vqline7 = ""
    line7 = urllib.unquote_plus(vqline7)
    vqline8 = _get_keyboard(heading="Enter line 8 of your feed" )
    if ( not vqline8 ): vqline8 = ""
    line8 = urllib.unquote_plus(vqline8)
    vqline9 = _get_keyboard(heading="Enter line 9 of your feed" )
    if ( not vqline9 ): vqline9 = ""
    line9 = urllib.unquote_plus(vqline9)
    vqline10 = _get_keyboard(heading="Enter line 10 of your feed" )
    if ( not vqline10 ): vqline10 = ""
    line10 = urllib.unquote_plus(vqline10)
...
def BuildAWizard():
    ...
    ReplaceText(addonxml,'addonname','plugin.program.'+id)
    ReplaceText(addonxml,'wizardname',name)
    ReplaceText(addonxml,'providername',proname)
    ReplaceText(addonxml,'zipurl',url)
    ReplaceText(defaultpy,'addonname','plugin.program.'+id)
    ReplaceText(defaultpy,'wizardname',name)
    ReplaceText(defaultpy,'providername', proname)
    ReplaceText(defaultpy,'zipurl',url)
    ...
    dialog.ok(Title, 'Your wizard is finished and saved to:','[COLOR deepskyblue]'+outputfolder+'[/COLOR]','You can replace the icon.png file to your own, just make sure it\'s called \'[COLOR deepskyblue]icon.png[/COLOR]\'')
    sys.exit()

def MKIVWIZARD():
    try:
        link = OPEN_URL('http://mkiv.netne.net/Admin/Files/Wizard.xml').replace('\n','').replace('\r','')
